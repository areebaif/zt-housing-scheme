...
export const TotalsSummary: React.FC<TotalsSummaryProps> = (props) => {
    ...
    const notSoldPlots = plots?.filter((element) => element.status === "not_sold");
    const partiallySold = plots?.filter((element) => element.status === "partially_paid");
    const fullySold = plots?.filter((element) => element.status === "fully_paid");
    const registryTransferred = plots?.filter((element) => element.status === "registry_transferred");
    // Render
    return (
